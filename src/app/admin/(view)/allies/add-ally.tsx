...
import { addAllyApi } from "@/lib/api/core";
...
export default function AddAlly() {
  const [cookies] = useCookies(["token"]);
  const qClient = useQueryClient();
  const { mutate } = useMutation({
    mutationKey: ["add_ally"],
    mutationFn: (body: AllyForm) => addAllyApi(body, cookies.token),
    onError: (err) => {
      toast.error(err.message ?? "Failed to complete this request");
    },
    onSuccess: (data) => {
      toast.success(data.message ?? "Successfully added a new ally");
  ...
  const form = useForm<AllyForm>({
    ...
    defaultValues: {
      name: "",
      location: "",
      type: "pub", // first value of type enum
      status: "active", // first value of status enum
      websiteURL: "",
      marketingBlurb: "",
      image: undefined,
    },
