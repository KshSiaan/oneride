...
export default function Guest({ id }: { id: string }) {
  ...
  const { mutate } = useMutation({
    ...
    mutationFn: (data: {
      ...
      return createBookingGuestApi(data);
    },
    onError: (err) => {
      toast.error(err.message ?? "Failed to complete this request");
    },
    onSuccess: (data: idk) => {
      ...
      createIntent(
        ...
        },
        {
          onSuccess: (succ: idk) => {
            navig.push(
              `/add-rides/details/payment?id=${data.data._id}&p_id=${succ.data._id}`
            );
          },
        }
      );
